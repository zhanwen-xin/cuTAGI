...
def estimate_hs_distribution_all_models(baseline_process_error_vars, num_steps,
    ...
    print('Estimating the multivariate normal distribution...')
    ...
    # Collect samples
    ARd_samples_all_models = []
    LLd_samples_all_models = []
    LTd_samples_all_models = []
    x_samples_all_models = []
    num_ts = 10
    for baseline_process_error_var in baseline_process_error_vars:
        ######################################## Meta-AR model ########################################
        ...
        ARd_samples_all_models.append(ARd_samples)
        LLd_samples_all_models.append(LLd_samples)
        LTd_samples_all_models.append(LTd_samples)
        x_samples_all_models.append(x_samples)

    # Reshape
    x_samples_all_models = np.array(x_samples_all_models).reshape((len(baseline_process_error_vars), num_ts, num_steps)).transpose(1, 2, 0).reshape(-1, len(baseline_process_error_vars)) 

    mean_x = np.mean(x_samples_all_models, axis=0)
    cov_x = np.cov(x_samples_all_models, rowvar=False)

    # Get the correlation matrix of x_samples
    x_samples_std = np.sqrt(np.diag(cov_x))
    x_samples_cor = cov_x / np.outer(x_samples_std, x_samples_std)
    print('Correlation matrix of x_samples:')
    print(x_samples_cor)

    return multivariate_normal(mean=mean_x, cov=cov_x)
...
def base_drift_candidate_model(baseline_process_error_var, trigger_pos, data_generator, num_steps, 
    ...
    for i in range(num_steps):
        ...
        LTd_na_all.append(xd_pred['mu'][1])
    ...
    return likelihood_x_na, likelihood_x_a, likelihood_y_na, likelihood_y_a, LTd_na_all, LTd_a_all
