...
class SyntheticTimeSeriesDataloader:
    """Data loader for time series"""
    ...
    def load_data_from_csv(self, data_file: str, skip_row = 0, select_column = None) -> pd.DataFrame:
        """Load data from csv file"""
    ...
    def process_data(self) -> dict:
        ...
        if self.ts_idx is not None:
            x = x[:,self.ts_idx:self.ts_idx+1]   # choose time series column
        date_time = self.load_data_from_csv(self.date_time_file, skip_row=1)
